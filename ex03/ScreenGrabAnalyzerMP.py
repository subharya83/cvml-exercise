...
class NewsVideoAnalyzer:
    ...
    def _setup_image_captioner(self):
        ...
        files_to_download = [
            "config.json", "pytorch_model.bin", "vocab.txt", "generation_config.json",
        ...
        for file in files_to_download:
            if not (model_path / file).exists():
                ...
                try:
                    hf_hub_download(repo_id=model_id, filename=file, local_dir=model_path)
    ...
    def analyze_video(self, video_path, output_path):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for i, (scene_start, scene_end) in enumerate(scenes, 1):
                ...
                future = executor.submit(self.process_scene, i, frame, scene_start, scene_end)
    ...
    def process_scene(self, scene_id, frame, scene_start, scene_end):
        ...
        if self.debug_dir:
            scene_debug_path = self.debug_dir / f"scene_{scene_id}_data.json"
            with open(scene_debug_path, 'w', encoding='utf-8') as f:
                json.dump(scene_data, f, indent=2, ensure_ascii=False)
            print(f"Saved scene {scene_id} debug data to {scene_debug_path}")
        
        return scene_data

def main():
    import argparse
    parser = argparse.ArgumentParser(description='Analyze news video and generate JSON output [MP version]')
